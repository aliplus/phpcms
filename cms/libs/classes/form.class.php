...
class form {
	 ...
	 */
	public static function editor($textareaid = 'content', $toolbar = 'basic', $toolvalue = '', $module = '', $catid = '', $color = '', $allowupload = 0, $allowbrowser = 1,$alowuploadexts = '',$height = 200,$disabled_page = 0, $autofloat = 0, $autoheight = 0, $theme = '', $watermark = 1, $image_reduce = '', $div2p = 0, $enablesaveimage = 1, $allowuploadnum = '10') {
		if ($autofloat) {
		...
		if (pc_base::load_config('system', 'editor')) {
			...
			if($allowupload) $str .= "filebrowserUploadUrl : 'index.php?m=attachment&c=attachments&a=ckeditor_upload&module=".$module."&catid=".$catid."&dosubmit=1&watermark_enable=".intval($watermark)."&image_reduce=".intval($image_reduce)."',\r\n";
			if($color) {
				$str .= "uiColor: '$color',";
			}
			$str .= "toolbar :\r\n";
			...
			if($toolbar == 'basic') {
				...
				$toolbar .= ", 'HighlightCode', '|', 'Horizontal', 'Date', 'Time', 'Spechars', 'WordImage', '|', 'InsertTable', 'DeleteTable', 'InsertParagraphBeforeTable', 'InsertRow', 'DeleteRow', 'InsertCol', 'DeleteCol', 'MergeCells', 'MergeRight', 'MergeDown', 'SplittoCells', 'SplittoRows', 'SplittoCols', '|', 'Print', 'Preview', 'SearchReplace', 'Help']";
			} elseif($toolbar == 'desc') {
			...
			$str .= "var editor = new baidu.editor.ui.Editor({UEDITOR_HOME_URL:'".WEB_PATH."statics/js/ueditor/',serverUrl:'".WEB_PATH."api.php?op=controller&catid=".intval($_GET['catid'])."&is_wm=".intval($watermark)."&is_esi=".intval($enablesaveimage)."&image_reduce=".intval($image_reduce)."',".join(",",$opt)."});editor.render('$textareaid');\n";
			$str .= '</script>';
		}
		...
		if($allowupload) {
			$authkey = upload_key("$allowuploadnum,$alowuploadexts,$allowbrowser,,,,$image_reduce");
			$ext_str.="<a onclick=\"flashupload('flashupload', '".L('attachmentupload')."','{$textareaid}','','{$allowuploadnum},{$alowuploadexts},{$allowbrowser},,,,{$image_reduce}','{$module}','{$catid}','{$authkey}');return false;\" href=\"javascript:void(0);\">".L('attachmentupload')."</a>";
		}
		$ext_str .= "</div>";
		if ($show_page=="true") {
	 ...
	 */
	public static function images($name, $id = '', $value = '', $moudle='', $catid='', $size = 50, $class = '', $ext = '', $alowexts = '',$thumb_setting = array(),$watermark_setting = 0, $image_reduce = '' ) {
		if(!$id) $id = $name;
		if(!$size) $size= 50;
		...
		$authkey = upload_key("1,$alowexts,1,$thumb_ext,$watermark_setting,$image_reduce");
		return $str."<input type=\"text\" name=\"$name\" id=\"$id\" value=\"$value\" size=\"$size\" class=\"$class\" $ext/>  <input type=\"button\" class=\"button\" onclick=\"javascript:flashupload('{$id}_images', '".L('attachmentupload')."','{$id}','submit_images','1,{$alowexts},1,{$thumb_ext},{$watermark_setting},{$image_reduce}','{$moudle}','{$catid}','{$authkey}')\"/ value=\"".L('imagesupload')."\">";
	}
	 ...
	 * @param int $image_reduce
	 */
	public static function upfiles($name, $id = '', $value = '', $moudle='', $catid='', $size = 50, $class = '', $ext = '', $alowexts = '',$file_setting = array(), $image_reduce = '' ) {
		if(!$id) $id = $name;
		if(!$size) $size= 50;
		...
		$authkey = upload_key("1,$alowexts,1,$file_ext,,$image_reduce");
	return $str."<input type=\"text\" name=\"$name\" id=\"$id\" value=\"$value\" size=\"$size\" class=\"$class\" $ext/>  <input type=\"button\" class=\"button\" onclick=\"javascript:flashupload('{$id}_files', '".L('attachmentupload')."','{$id}','submit_attachment','1,{$alowexts},1,{$file_ext},,{$image_reduce}','{$moudle}','{$catid}','{$authkey}')\"/ value=\"".L('filesupload')."\">";
	}
