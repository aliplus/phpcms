 ...
 * @return 分页
 */
function wappages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 10) {
	if(defined('URLRULE') && $urlrule == '') {
		$urlrule = URLRULE;
		$array = $GLOBALS['URL_ARRAY'];
	} elseif($urlrule == '') {
	...
	if (!$siteid) $siteid = 1;
	$sitelist = getcache('sitelist','commons');
	if ($sitelist[$siteid]['waphtml']==1 && defined('ISHTML')) {
	...
	if($num > $perpage) {
		...
		$multipage .= '<a class="a1">'.$num.L('page_item').'</a>';
		if($curr_page>0) {
			$multipage .= ' <a href="'.$wap_root.pageurl($urlrule, $curr_page-1, $array).'" class="a1">'.L('previous').'</a>';
			if($curr_page==1) {
				$multipage .= ' <span>1</span>';
			} elseif($curr_page>6 && $more) {
				$multipage .= ' <a href="'.$wap_root.pageurl($urlrule, 1, $array).'">1</a>..';
			} else {
				$multipage .= ' <a href="'.$wap_root.pageurl($urlrule, 1, $array).'">1</a>';
			}
		}
		for($i = $from; $i <= $to; $i++) {
			if($i != $curr_page) {
				$multipage .= ' <a href="'.$wap_root.pageurl($urlrule, $i, $array).'">'.$i.'</a>';
			} else {
				$multipage .= ' <span>'.$i.'</span>';
			}
		}
		if($curr_page<$pages) {
			if($curr_page<$pages-5 && $more) {
				$multipage .= ' ..<a href="'.$wap_root.pageurl($urlrule, $pages, $array).'">'.$pages.'</a> <a href="'.$wap_root.pageurl($urlrule, $curr_page+1, $array).'" class="a1">'.L('next').'</a>';
			} else {
				$multipage .= ' <a href="'.$wap_root.pageurl($urlrule, $pages, $array).'">'.$pages.'</a> <a href="'.$wap_root.pageurl($urlrule, $curr_page+1, $array).'" class="a1">'.L('next').'</a>';
			}
		} elseif($curr_page==$pages) {
			$multipage .= ' <span>'.$pages.'</span> <a href="'.$wap_root.pageurl($urlrule, $curr_page, $array).'" class="a1">'.L('next').'</a>';
		} else {
			$multipage .= ' <a href="'.$wap_root.pageurl($urlrule, $pages, $array).'">'.$pages.'</a> <a href="'.$wap_root.pageurl($urlrule, $curr_page+1, $array).'" class="a1">'.L('next').'</a>';
		}
	}
...
function get_memberavatar($uid, $size = '') {
	...
	if ($avatar_db) {
		if ($size) {
			$avatar = $image->thumb(pc_base::load_config('system', 'upload_url').$avatar_db['filepath'], $size, $size, 0, 'auto', 0);
		} else {
			$avatar = pc_base::load_config('system', 'upload_url').$avatar_db['filepath'];
		}
	} else {
 ...
 * @param  $webimg 剪切网络图片
 */
function thumb($imgurl, $width = 100, $height = 100 ,$water = 0, $mode = 'auto', $webimg = 0) {
	global $image;
	if(!is_object($image)) {
		pc_base::load_sys_class('image');
		$image = new image();
	}
	return $image->thumb($imgurl, $width, $height, $water, $mode, $webimg);
}
 ...
 * @param $siteid   站点id
 */
function sitewapurl($siteid) {
	static $sitelist;
	if(!$siteid) return WEB_PATH.'index.php?m=wap';
	if(empty($sitelist)) $sitelist = getcache('sitelist','commons');
	if(!substr($sitelist[$siteid]['mobile_domain'],0,-1)) return substr($sitelist[$siteid]['domain'],0,-1).WEB_PATH.'index.php?m=wap';
	/*if ($sitelist[$siteid]['waphtml']==1 || $sitelist[$siteid]['mobile_domain']) {
		return substr($sitelist[$siteid]['mobile_domain'],0,-1);
	} else {
		return substr($sitelist[$siteid]['domain'],0,-1).WEB_PATH.'index.php?m=wap';
	}*/
