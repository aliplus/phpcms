...
class content extends admin {
	...
	public function init() {
		...
		if(isset($_GET['catid']) && $_GET['catid'] && $this->categorys[$_GET['catid']]['siteid']==$this->siteid) {
			...
			$datas = $this->db->listinfo($where,'id desc',$_GET['page'],$pagesize);
			$pages = $this->db->pages;
			...
			for($i=1;$i<=$workflow_steps;$i++) {
				...
				$workflow_menu .= '<a href="?m=content&c=content&a=&menuid='.$_GET['menuid'].'&catid='.$catid.'&steps='.$i.'&pc_hash='.$pc_hash.'" class="layui-btn'.$current.' layui-btn-sm"><em>'.L('workflow_'.$i).$newimg.'</em></a><span>|</span>';
			}
			if($workflow_menu) {
				$current = isset($_GET['reject']) ? ' layui-btn-danger' : '';
				$workflow_menu .= '<a href="?m=content&c=content&a=&menuid='.$_GET['menuid'].'&catid='.$catid.'&pc_hash='.$pc_hash.'&reject=1" class="layui-btn'.$current.' layui-btn-sm"><em>'.L('reject').'</em></a><span>|</span>';
			}
	...
	public function initall() {
		...
		for($i=1;$i<=$workflow_steps;$i++) {
			...
			$current = $steps==$i ? ' layui-btn-danger' : '';
			$r = $this->db->get_one(array('catid'=>$catid,'status'=>$i));
			$newimg = $r ? '<img src="'.IMG_PATH.'icon/new.png" style="padding-bottom:2px" onclick="window.location.href=\'?m=content&c=content&a=&menuid='.$_GET['menuid'].'&catid='.$catid.'&steps='.$i.'&pc_hash='.
...
$pc_hash.'\'">' : '';
	...
	public function add() {
		if(isset($_POST['dosubmit']) || isset($_POST['dosubmit_continue'])) {
			...
			if($category['type']==0) {
				...
				// 去除站外链接
				$value = str_replace('\\', '', $_POST['info']['content']);
				...
				$_POST['info']['content'] = $value;
				$this->db->add_content($_POST['info']);
				if(isset($_POST['dosubmit'])) {
					showmessage(L('add_success').L('2s_close'),'blank','','','parent.right.location.reload();function set_time() {$("#secondid").html(1);}setTimeout("set_time()", 500);setTimeout("ownerDialog.close()", 1200);');
				} else {
	...
	public function edit() {
			...
			if(isset($_POST['dosubmit']) || isset($_POST['dosubmit_continue'])) {
				...
				if(isset($_POST['dosubmit'])) {
					showmessage(L('update_success').L('2s_close'),'blank','','','parent.right.location.reload();function set_time() {$("#secondid").html(1);}setTimeout("set_time()", 500);setTimeout("ownerDialog.close()", 1200);');
				} else {
	...
	public function delete() {
		if(isset($_GET['dosubmit'])) {
			$catid = intval($_GET['catid']);
			if(!$catid) exit(json_encode(array('code' => 0, 'msg' => L('missing_part_parameters'))));
			$modelid = $this->categorys[$catid]['modelid'];
			...
			if(empty($_POST['ids'])) exit(json_encode(array('code' => 0, 'msg' => L('you_do_not_check'))));
			//附件初始化
			...
			foreach($_POST['ids'] as $id) {
				...
				if($content_ishtml && !$r['islink']) {
					...
					$fileurl = $urls[1];
					$wapfileurl = pc_base::load_config('system','wap_root').$urls[1];
					...
					$waplasttext = strrchr($wapfileurl,'.');
					$waplen = -strlen($waplasttext);
					$wappath = substr($wapfileurl,0,$waplen);
					$wappath = ltrim($wappath,'/');
					$wapfilelist = glob(CMS_PATH.$wappath.'{_,-,.}*',GLOB_BRACE);
					...
					if($sitelist[$this->siteid]['waphtml']==1) {
						foreach ($wapfilelist as $wapdelfile) {
							$waplasttext = strrchr($wapdelfile,'.');
							if(!in_array($waplasttext, array('.htm','.html','.shtml'))) continue;
							@unlink($wapdelfile);
						}
					}
				} else {
			...
			//更新栏目统计
			$this->db->cache_items();
			exit(json_encode(array('code' => 1, 'msg' => L('operation_success'))));
		} else {
			exit(json_encode(array('code' => 0, 'msg' => L('operation_failure'))));
		}
	}
	/**
	 * 更新
	...
	public function recycle() {
		if(isset($_GET['dosubmit'])) {
			...
			if ($catid) {
				...
				foreach($_POST['ids'] as $id) {
					if ($recycle) {
						$this->db->update(array('status'=>0),array('id'=>$id));
					} else {
	...
	public function update() {
		if(isset($_GET['dosubmit'])) {
			...
			if ($catid) {
				$modelid = $this->categorys[$catid]['modelid'];
				$this->db->set_model($modelid);
				$this->db->update(array($_POST['field']=>$_POST['value']),array('id'=>$_POST['id']));
				exit(json_encode(array('code' => 1, 'msg' => L('operation_success'))));
			} else {
	...
	public function pass() {
		...
		if($workflowid) {
			...
			if($_SESSION['roleid']!=1 && $steps && !in_array($steps,$admin_privs)) exit(json_encode(array('code' => 0, 'msg' => L('permission_to_operate'))));
			//更改内容状态
	...
	*/
		...
		if(!empty($this->categorys)) {
			foreach($this->categorys as $r) {
				...
				if($r['type']==1 || $from=='block') {
					...
					$r['icon_type'] = 'file';
					$r['category_edit'] = " <a href='?m=content&c=content&a=add&menuid=".$_GET['menuid']."&catid=".$r['catid']."' target='right' onclick='open_list(this)'>".L('edit')."</a>";
					$r['add_icon'] = '';
					$r['type'] = 'add';
				} else {
					...
					$r['category_edit'] = '';
					$r['type'] = 'init';
					$r['add_icon'] = "<a target='right' href='?m=content&c=content&menuid=".$_GET['menuid']."&catid=".$r['catid']."' onclick=\"javascript:contentopen('?m=content&c=content&a=add&menuid=".$_GET['menuid']."&catid=".$r['catid']."&hash_page=".$_SESSION['hash_page']."','".L('add_content')."')\"><img src='".IMG_PATH."add_content.gif' alt='".L('add')."'></a> ";
				}
				$categorys[$r['catid']] = $r;
			}
		}
	...
	public function public_crop() {
		...
		if(isset($_POST['filepath'])){
			...
			$image = new image();
			$filepath = pc_base::load_config('system', 'upload_path');
			$filename = date('Y/md/').date("ymdhis").rand(100,999);
			//$new_file = date('Y/md/').'thumb_'.$w.'_'.$h.'_'.basename($_POST['filepath']);
			$filetype = fileext($_POST['filepath']);
			$fileinfo = $image->crops(CMS_PATH.$_POST['filepath'], $filepath.$filename.'.'.$filetype, $w, $h, $x, $y);
			//$fileinfo = $image->crops(CMS_PATH.$_POST['filepath'], $filepath.$new_file, $w, $h, $x, $y);
			if($fileinfo){
				$this->att_db = pc_base::load_model('attachment_model');
				$uploadedfile['module'] = $_GET['module'];
				$uploadedfile['catid'] = $_GET['catid'];
				$uploadedfile['siteid'] = $this->get_siteid();
				...
				$uploadedfile['filename'] = strlen(basename($_POST['filepath']))>49 ? date('Ymdhis').rand(100, 999).'.'.$filetype : basename($_POST['filepath']);
				$uploadedfile['filesize'] = $fileinfo['size'];
				$uploadedfile['fileext'] = $filetype;
				$aid = $this->att_db->api_add($uploadedfile);
				exit(json_encode(array('status' => 1, 'filepath' => pc_base::load_config('system', 'upload_url').$filename.'.'.$filetype)));
				//exit(json_encode(array('status' => 1, 'filepath' => pc_base::load_config('system', 'upload_url').$new_file)));
			}else{
		...
		if (isset($_GET['picurl']) && !empty($_GET['picurl'])) {
			...
			$show_header =  '';
			$filepath = isset($_GET['picurl']) ? base64_decode($_GET['picurl']) : showmessage(L('lose_parameters'));
			if(strpos($filepath, '://')) $filepath = strstr($filepath, 'uploadfile');
			if(!is_file(CMS_PATH.$filepath)) showmessage('请选择本地已存在的图像！');
			$spec = isset($_GET['spec']) ? intval($_GET['spec']) : 1; 
			$catid = intval($_GET['catid']);
			$input = isset($_GET['input']) ? $_GET['input'] : 'thumb';
			$preview = isset($_GET['preview']) ? $_GET['preview'] : 'thumb';
			switch ($spec){
				case 1:
				  $spec = '4 / 3';
				  break;  
	...
	public function public_preview() {
		...
		if ($steps) {
			...
			function ajax_manage(type) {
				...
				setInterval('window.opener.location.reload();window.close();', 2000);
			}
			function set_time() {
				$('#secondid').html(1);
			}
	...
	public function ajax_pass() {
		...
		$modelid = $this->categorys[$catid]['modelid'];
		$this->db->set_model($modelid);
		$this->db->update(array('status'=>99),array('id'=>$_GET['id']));
		exit(json_encode(array('code' => 1, 'msg' => L('operation_success'))));
	}
