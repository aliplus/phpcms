...
class html {
	private $siteid,$url,$html_root,$wap_root,$queue,$categorys;
	public function __construct() {
		...
		$this->wap_root = pc_base::load_config('system','wap_root');
	...
	public function show($file, $data = '', $array_merge = 1,$action = 'add',$upgrade = 0) {
		...
		if($paginationtype!=0) {
			...
			if($CONTENT_POS !== false) {
				...
				foreach ($pageurls as $page=>$urls) {
					...
					$wappages = wap_content_pages($pagenumber,$page, $pageurls ,$this->siteid , $CAT['setting']['content_ishtml']);
					//判断[page]出现的位置是否在第一位 
					if($CONTENT_POS<7) {
					...
					$wappagefile = CMS_PATH.$this->wap_root.'/'.str_replace(CMS_PATH,'',$pagefile);
					ob_start();
					include template('content', $template);
					$this->createhtml($pagefile);
					if($this->sitelist[$this->siteid]['waphtml']==1) {
						ob_start();
						include template('wap', $template);
						$this->createhtml($wappagefile);
					}
				}
				return true;
			}
		...
		//分页处理结束
		$file = CMS_PATH.$file;
		$wapfile = CMS_PATH.$this->wap_root.'/'.str_replace(CMS_PATH,'',$file);
		ob_start();
		include template('content', $template);
		...
		if($this->sitelist[$this->siteid]['waphtml']==1) {
			ob_start();
			include template('wap', $template);
			return $this->createhtml($wapfile);
		}
	}
	...
	public function category($catid, $page = 0) {
		...
		if($create_to_html_root) {
			if($this->siteid==1) {
				...
				$wapfile = CMS_PATH.$this->wap_root.'/'.$base_file;
			} else {
				$file = CMS_PATH.substr($this->html_root,1).$base_file;
				$wapfile = CMS_PATH.$this->wap_root.'/'.substr($this->html_root,1).$base_file;
			}
			//添加到发布点队列
	...
	public function index() {
		if($this->siteid==1) {
			...
			$wapfile = CMS_PATH.$this->wap_root.'/index.html';
			$mapfile = CMS_PATH.$this->wap_root.'/map.html';
			//添加到发布点队列
			$this->queue->add_queue('edit','/index.html',$this->siteid);
		} else {
			$site_dir = $this->sitelist[$this->siteid]['dirname'];
			$file = $this->html_root.'/'.$site_dir.'/index.html';
			$wapfile = $this->wap_root.'/'.$this->html_root.'/'.$site_dir.'/index.html';
			$mapfile = $this->wap_root.'/'.$this->html_root.'/'.$site_dir.'/map.html';
			//添加到发布点队列
			$this->queue->add_queue('edit',$file,$this->siteid);
			$file = CMS_PATH.$file;
			$wapfile = CMS_PATH.$wapfile;
		...
		if($this->sitelist[$siteid]['waphtml']==1) {
			ob_start();
			include template('wap','maps',$style);
			$this->createhtml($mapfile);
			ob_start();
			include template('wap','index',$style);
			$this->createhtml($wapfile);
		}
