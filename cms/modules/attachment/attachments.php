...
class attachments {
	...
	public function upload() {
		...
		$a = $attachment->upload('upload',$site_allowext,'','','',intval($_GET['watermark_enable']),intval($_GET['image_reduce']));
		if($a){
	...
	public function ckeditor_upload() {
		...
		$attachment = new attachment($module,$catid,$siteid);
		$attachment->set_userid($this->userid);
		$a = $attachment->upload('upload',$site_allowext,'','','',$watermark,intval($_GET['image_reduce']));
		if($a){
			$filepath = $attachment->uploadedfiles[0]['filepath'];
			$fn = intval($_GET['CKEditorFuncNum']);
			$this->upload_json($a[0],$filepath,$attachment->uploadedfiles[0]['filename']);
	...
	public function h5upload(){
		...
		if(isset($_POST['dosubmit'])){
			if( $_POST['swf_auth_key'] != md5(pc_base::load_config('system','auth_key').$_POST['h5uploadSESSID']) || ($_POST['isadmin']==0 && !$grouplist[$_POST['groupid']]['allowattachment'])) exit();
			pc_base::load_sys_class('attachment','',0);
			...
			$site_setting = get_site_setting($siteid);
			$site_allowext = $site_setting['upload_allowext'];
			$aids = $attachment->upload('Filedata',$site_allowext,'','',array($_POST['thumb_width'],$_POST['thumb_height']),$_POST['watermark_enable'],$_POST['image_reduce']);
			if($aids[0]) {
				...
				if($attachment->uploadedfiles[0]['isimage']) {
					echo $aids[0].','.$this->upload_url.$attachment->uploadedfiles[0]['filepath'].','.$attachment->uploadedfiles[0]['isimage'].','.$filename.','.$attachment->size($attachment->uploadedfiles[0]['filesize']);
				} else {
					...
					echo $aids[0].','.$this->upload_url.$attachment->uploadedfiles[0]['filepath'].','.$fileext.','.$filename.','.$attachment->size($attachment->uploadedfiles[0]['filesize']);
				}
				exit;
			} else {
