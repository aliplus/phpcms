...
class index {
	function __construct() {		
		...
		define('WAP_SITEURL', sitewapurl($this->siteid) ? sitewapurl($this->siteid).'index.php?' : APP_PATH.'index.php?m=wap&siteid='.$this->siteid);
	}
	...
	public function lists() {
		...
		if($type==0) {
			...
			if ($sitelist[$siteid]['waphtml']==1 && ISHTML) {
				...
				$urlrules = 'index.php?m=wap&c=index&a=lists&catid={$catid}~index.php?m=wap&c=index&a=lists&catid={$catid}&page={$page}';
			}
			//构造wap url规则
	...
	public function show() {
		...
		if($rs['paginationtype']!=0) {
			...
			if($CONTENT_POS !== false) {
				...
				//当不存在 [/page]时，则使用下面分页
				$wappages = wap_content_pages($pagenumber,$page, $pageurls);
				//判断[page]出现的位置是否在第一位 
				if($CONTENT_POS<7) {
