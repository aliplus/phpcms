...
class wap_url{
	private $urlrules,$categorys,$html_root,$wap_root;
	...
	*/
	public function show($id, $page = 0, $catid = 0, $time = 0, $prefix = '',$data = '',$action = 'edit',$upgrade = 0) {
		...
		if ($sitelist[$siteid]['waphtml']==1 && $content_ishtml) {
			$wap_root = $this->wap_root;
		}
		//当内容为转换或升级时
		if($upgrade || (isset($_POST['upgrade']) && defined('IN_ADMIN') && $_POST['upgrade'])) {
			...
			if ($sitelist[$siteid]['waphtml']==1 && $content_ishtml) {
				...
				$urlrules = 'index.php?m=wap&c=index&a=show&catid={$catid}&id={$id}|index.php?m=wap&c=index&a=show&catid={$catid}&id={$id}&page={$page}';
			}
			...
			if($content_ishtml && $url) {
				if ($domain_dir && $category['isdomain']) {
					$url_arr[1] = $wap_root.$html_root.'/'.$domain_dir.$urls;
					$url_arr[0] = $wap_root.$url.$urls;
				} else {
					$url_arr[1] = $wap_root.$html_root.'/'.$urls;
					$url_arr[0] = WEB_PATH == '/' ? $match_url.$wap_root.$html_root.'/'.$urls : $match_url.rtrim(WEB_PATH,'/').$wap_root.$html_root.'/'.$urls;
				}
			} elseif($content_ishtml) {
				$url_arr[0] = WEB_PATH == '/' ?  $wap_root.$html_root.'/'.$urls : rtrim(WEB_PATH,'/').$wap_root.$html_root.'/'.$urls;
				$url_arr[1] = $wap_root.$html_root.'/'.$urls;
			} else {
				$url_arr[0] = $url_arr[1] = $wap_root.$urls;
			}
		}
