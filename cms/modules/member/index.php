...
class index extends foreground {
	...
	public function account_manage() {
		...
		foreach($model_info as $k=>$v) {
			...
			if($v['formtype'] == 'image') {
				$member_modelinfo[$v['name']] = "<a href='$member_modelinfo_arr[$k]' target='_blank'><img src='$member_modelinfo_arr[$k]' height='40' widht='40' onerror=\"this.src='/statics/images/member/nophoto.gif'\"></a>";
				...
				if(is_array($tmp)) {
					foreach ($tmp as $tv) {
						$member_modelinfo[$v['name']] .= " <a href='$tv[url]' target='_blank'><img src='$tv[url]' height='40' widht='40' onerror=\"this.src='/statics/images/member/nophoto.gif'\"></a>";
					}
					unset($tmp);
				}
	...
	public function uploadavatar() {
		...
		$avatarfile = pc_base::load_config('system', 'upload_path');
		if(!file_exists($avatarfile.'avatar/'.date('Y/md/'))) {
			mkdir($avatarfile.'avatar/'.date('Y/md/'), 0777, true);
		}
		if(preg_match('/^(data:\s*image\/(\w+);base64,)/', $base64_img, $result)){
			$type = $result[2];
			if(in_array($type,array('pjpeg','jpeg','jpg','gif','bmp','png'))){
				$filename = date('YmdHis').'.'.$type;
				$new_file = 'avatar/'.date('Y/md/').$filename;
				if(file_put_contents($avatarfile.$new_file, base64_decode(str_replace($result[1], '', $base64_img)))){
					$img_path = str_replace('../../..', '', $new_file);
					$this->att_db = pc_base::load_model('attachment_model');
					$uploadedfile['module'] = 'member';
					$uploadedfile['catid'] = '0';
					$uploadedfile['siteid'] = $siteid;
					$uploadedfile['userid'] = $uid;
					...
					$uploadedfile['authcode'] = md5($new_file);
					$uploadedfile['isimage'] = in_array($type, array('gif', 'jpg', 'jpeg', 'png', 'bmp')) ? 1 : 0;
					$uploadedfile['filepath'] = $new_file;
					$uploadedfile['filename'] = strlen($filename)>49 ? date('Ymdhis').rand(100, 999).'.'.$type : $filename;
					$uploadedfile['filesize'] = filesize($avatarfile.$new_file);
					$uploadedfile['fileext'] = $type;
					$aid = $this->att_db->api_add($uploadedfile);
					$this->db->update(array('avatar'=>$aid), array('userid'=>$this->memberinfo['userid']));
					$this->att_db->update(array('status'=>0), array('userid'=>$uid,'status'=>1));
					$this->att_db->update(array('status'=>1), array('aid'=>$aid));
					$arr = array('status'=>1,'msg'=>'图片上传成功'.pc_base::load_config('system', 'upload_url').$img_path);
					echo json_encode($arr);
				}else{
					$arr = array('status'=>0,'msg'=>'图片上传失败');
					echo json_encode($arr);
				}
			}else{
				$arr = array('status'=>0,'msg'=>'图片上传类型错误');
				echo json_encode($arr);
			}
		}else{
			$arr = array('status'=>0,'msg'=>'文件错误');
			echo json_encode($arr);
		}
	}
