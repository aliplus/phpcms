...
class site extends admin {
	...
	public function init() {
		...
		if (!$this->db->field_exists('waphtml')) {
			$this->db->query('ALTER TABLE `'.$tablename.'` ADD `waphtml` tinyint(1) unsigned NOT NULL DEFAULT \'0\' COMMENT \'生成静态\' AFTER `mobileauto`');
		}
		if (!$this->db->field_exists('not_pad')) {
			$this->db->query('ALTER TABLE `'.$tablename.'` ADD `not_pad` tinyint(1) unsigned NOT NULL DEFAULT \'0\' COMMENT \'将平板端排除\' AFTER `waphtml`');
		}
	...
	public function add() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			$waphtml = isset($_POST['waphtml']) && trim($_POST['waphtml']) ? trim($_POST['waphtml']) : 0;
