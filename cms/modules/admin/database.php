...
class database extends admin {
		...
		if($_GET['dosubmit']) {
			...
			if(is_array($sqlfiles)) {
				...
				foreach($sqlfiles as $id=>$sqlfile) {
					//老的数据库备份文件转换为新格式
					if(preg_match("/([cmstables_|db_][0-9]{8}_[0-9a-z]{20}_)([0-9]+)\.sql/i",basename($sqlfile),$num)) {
					...
					if(preg_match("/([0-9a-z]{20}_[cmstables_|db_]+[0-9]{8}_)([0-9]+)\.sql/i",basename($sqlfile),$num)) {
	...
	public function public_repair() {
		...
		if($tables && in_array($operation,array('repair','optimize','flush'))) {
			...
			foreach ($_POST['tables'] as $table) {
				$this->db->query("ALTER TABLE `$table` DEFAULT CHARSET=utf8 COLLATE utf8_unicode_ci");
			}
			showmessage(L('operation_success'),'?m=admin&c=database&a=export&menuid=914&pdoname='.$pdo_name);
	...
	public function public_add() {
		...
		$ids = $_POST['tables'];
		if (!$ids) {
			exit(json_encode(array('code' => 0,'msg' => L('database_no_table'),'data' => '')));
		}
		$cache = array();
		$count = count($ids);
		if ($count > 100) {
		...
		exit(json_encode(array('code' => 1,'msg' => 'ok','data' => array('url' => '?m=admin&c=database&a=public_count_index&operation='.$operation.'&pdo_name='.$pdo_name))));
    }
	...
	public function public_count_index() {
		...
		$operation = $_GET['operation'];
		$pdo_name = $_GET['pdo_name'];
		$todo_url = '?m=admin&c=database&a=public_todo_index&operation='.$operation.'&pdo_name='.$pdo_name;
	...
	public function public_todo_index() {
		...
		if (!$cache) {
			exit(json_encode(array('code' => 0,'msg' => L('database_cache'),'data' => '')));
		}
		$data = $cache[$page];
		if ($data) {
			$html = '';
			...
			exit(json_encode(array('code' => $page + 1,'msg' => $html,'data' => '')));
		}
		// 完成
		delcache('db-todo-'.$operation, 'commons');
		exit(json_encode(array('code' => 100,'msg' => '','data' => '')));
    }
	/**
	 * 数据库导出方法
	...
	private function export_database($tables,$sqlcompat,$sqlcharset,$sizelimit,$action,$fileid,$random,$tableid,$startfrom,$tabletype) {
		...
		for($i = $tableid; $i < count($tables) && strlen($tabledump) < $sizelimit * 1000; $i++) {
			...
			$offset = 100;
			if(!$startfrom) {
				if($tables[$i]!=$this->db_tablepre.'session') {
				...
				if($tables[$i]==$this->db_tablepre.'session') {
					$tabledump = str_replace("CREATE TABLE `".$this->db_tablepre."session`", "CREATE TABLE IF NOT EXISTS `".$this->db_tablepre."session`", $tabledump);
				}
			}

			$numrows = $offset;
			while(strlen($tabledump) < $sizelimit * 1000 && $numrows == $offset) {
				if($tables[$i]==$this->db_tablepre.'session' || $tables[$i]==$this->db_tablepre.'member_cache') break;
