    ...
    public function _safe_check($file_ext, $data, $is_ext = 1) {
        ...
        if ($is_ext) {
            if (in_array($file_ext, $this->notallowed)) {
                return $this->dr_return_data(0, $this->getStateInfo("ERROR_SYSTEM_TYPE_NOT_ALLOWED"));
            } elseif (!$file_ext) {
                return $this->dr_return_data(0, L('无法读取文件扩展名'));
            }
        }

        // 验证扩展名格式
        if (!preg_match('/^[a-z0-9]+$/i', $file_ext)) {
            return $this->dr_return_data(0, L('此文件扩展名'.$file_ext.'不安全，禁止上传'));
        }
        ...
        if (in_array($file_ext, array('jpg', 'jpeg', 'png', 'gif'))) {
            ...
            if (strlen($data) < 100) {
                return $this->dr_return_data(0, L('图片文件不规范'));
            } elseif (strpos($data, '<?php') !== false) {
                return $this->dr_return_data(0, L('此图片不安全，禁止上传'));
        ...
        return $this->dr_return_data(1, 'ok');
    }

    /**
     * 数据返回统一格式
     */
    public function dr_return_data($code, $msg = '', $data = array()) {
        return array(
            'code' => $code,
            'msg' => $msg,
            'data' => $data,
